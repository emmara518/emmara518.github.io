    ...
    <div className="print-area space-y-6">
      ...
      <header className="flex flex-wrap items-end justify-between gap-3 border-b border-line pb-4">
        <div className="space-y-1">
          ...
          <h1 className="text-lg font-black text-ink">{child.name}</h1>
      ...
      <section className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4 [&>*]:min-w-0">
          <Card className="px-4 py-3.5 print:border-0 print:shadow-none print:bg-transparent">
            <span className="block font-mono text-xl font-black text-ink">{enrollments.length}</span>
            <span className="mt-0.5 block text-[11px] font-bold text-muted">عدد الكورسات</span>
          </Card>
          <Card className="px-4 py-3.5 print:border-0 print:shadow-none print:bg-transparent">
            <span className="block font-mono text-xl font-black text-ink">{avgProgress}%</span>
            <span className="mt-0.5 block text-[11px] font-bold text-muted">متوسط التقدم</span>
          </Card>
          <Card className="px-4 py-3.5 print:border-0 print:shadow-none print:bg-transparent">
            <span className="block font-mono text-xl font-black text-ink">{recentAttempts.length}</span>
            <span className="mt-0.5 block text-[11px] font-bold text-muted">عدد الاختبارات المُختبرة</span>
          </Card>
          <Card className="px-4 py-3.5 print:border-0 print:shadow-none print:bg-transparent">
            <span className="block font-mono text-sm font-extrabold text-ink">
              {lastActivity ? formatDate(lastActivity) : "—"}
            </span>
            <span className="mt-0.5 block text-[11px] font-bold text-muted">آخر نشاط</span>
          </Card>
        </div>
        {avgPercent !== null ? (
        ...
        {enrollments.length === 0 ? (
          ...
          <Card className="overflow-hidden print:border-0 print:shadow-none print:bg-transparent">
            <table className="w-full text-start text-sm">
              <thead>
                <tr className="border-b border-line bg-surface2/60 text-[11px] font-bold text-muted print:bg-transparent">
              ...
              <tbody>
                {enrollments.map((e) => (
                  <tr key={e.courseId} className="border-b border-line/60 last:border-0">
                    ...
                    <td className="whitespace-nowrap px-4 py-3.5 text-[11px] font-semibold text-muted">
      ...
      {/* Footer */}
      <footer className="space-y-1 border-t border-line pt-4 text-[11px] font-semibold text-muted">
        <p>
