          ...
          <div className="space-y-4">
            ...
            <div className="sticky top-14 z-10 -mx-1 lg:hidden">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-full border border-line bg-surface px-4 py-2 text-sm font-bold text-ink shadow-sm hover:border-brand hover:text-brand"
              >
                <ListVideo size={13} />
                قائمة الدروس
                <ChevronDown size={13} />
            ...
            {current?.youtubeVideoId ? (
              <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  <div>
                    ...
                    {room.course.requireSequentialProgress && (
                      <p className="mt-1 inline-flex items-center gap-1.5 rounded-lg bg-brand/10 px-2 py-1 text-xs font-bold text-brand">
                ...
                {viewNotice && (
                  <p className="flex items-center gap-2 rounded-xl border border-danger/30 bg-danger/10 px-3.5 py-2.5 text-xs font-bold text-danger">
                ...
                <ul className="space-y-2 text-base">
                  {room.exams.map((e) => (
                    <li key={e.id} className="flex items-center justify-between rounded-xl bg-surface2/60 px-3.5 py-2.5">
                      <span>{e.title}</span>
                      {room.enrolled ? (
                        <Link href={`/dashboard/exams/${e.id}`} className="text-sm font-bold text-brand hover:underline">
                          ابدأ ←
                        </Link>
                      ) : (
                          ...
                          return (
                            ...
                            >
                              ...
                              {isSequenceLocked ? null : (
                                <span className="font-mono text-xs text-muted">{formatDuration(v.durationSec)}</span>
                              )}
                            </button>
                          );
                        })}
